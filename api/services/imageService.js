...
class ImageService {
  constructor() {
    ...
    this.fs = fs; // Store fs.promises reference
  }
  ...
  async init() {
    try {
      ...
      debug('Image service initialized');
      return true;
    } catch (error) {
      debug(`Image service initialization failed: ${error.message}`);
      return false;
    }
  }
  ...
  async ensureDirectory(dirPath) {
    try {
      ...
      debug(`Failed to create directory: ${error.message}`);
      throw error;
    }
  }
  ...
  async readImageFile(filePath) {
    try {
      ...
      debug(`Failed to read image file: ${error.message}`);
      throw error;
    }
  }
  ...
  async writeImageFile(filePath, data) {
    try {
      ...
      debug(`Image file written: ${filePath}`);
    } catch (error) {
      debug(`Failed to write image file: ${error.message}`);
      throw error;
    }
  }
  ...
  async optimizeImage(inputPath, outputPath, options = {}) {
    ...
    try {
      // Read input file using promises
      const inputBuffer = await this.readImageFile(inputPath);
      ...
      debug(`Image optimized: ${path.basename(outputPath)}`);
      return true;
    } catch (error) {
      debug(`Image optimization failed: ${error.message}`);
      throw error;
    }
  }
  ...
  async cleanup() {
    this.initialized = false;
    debug('Image service cleaned up');
  }
  ...
  async getOpenGraphImage(url, retryCount = 0) {
    try {
      ...
      if (response.status === 429) {
        const retryAfter =
          parseInt(response.headers['retry-after']) || RETRY_DELAY;
  ...
  async getImageForArticle(url) {
    try {
      ...
      if (
        metadata.imageUrl &&
        (await this.validateImageUrl(metadata.imageUrl))
      ) {
