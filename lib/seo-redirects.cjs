...
async function getSeoRedirects() {
  return [
    ...
    // Legacy HTML pages only — do NOT redirect /callcenterhelper/api/* (live Next routes)
    {
      source: '/callcenterhelper/:page(contact|privacy|terms|settings)',
...
function seoRedirectMiddleware(req, res, next) {
  ...
  // Preserve Next.js API routes under /callcenterhelper/api/*
  if (path.startsWith('/callcenterhelper/api')) {
    return next();
  }
